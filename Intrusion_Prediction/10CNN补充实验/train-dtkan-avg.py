...
class FlowDataset(Dataset):
    def __init__(self, features, labels, transform=None):
        self.features = features  # 形状: (总样本数, 32, 6)
...
for label, typecsv in category_files.items():
    ...
    for flow_id, group in grouped:
        feature = group.iloc[:, 2:8].values  # 提取特征
...
best_model_path = 'best_dtkan_avg.pth'
model = CTKAN(num_classes=10).to(device)  # 假设你有 10 个类别
model.load_state_dict(torch.load('best_dtkan_avg.pth'))
'''
