...
# 初始化模型、损失函数和优化器
model = MLP(feature_num, hidden_dim, num_layer)
...
def train_mlp():
    ...
    # 训练结束后保存最佳模型
    torch.save(best_model_state, 'best_MLP32.pth')
    print("Best model saved as best_MLP32.pth")
...
best_model.load_state_dict(torch.load('best_MLP32.pth'))
best_model.eval()
