...
# 数据准备
print("Loading Data ...")
purchase_seq = pd.read_csv('third_packet_processed.csv', usecols=['timestamp'] + target_columns, index_col='timestamp')
...
# 初始化模型、损失函数和优化器
model = TKAN(input_size, rnn_unit, kan_layer_hidden, output_size)
...
def train_tkan():
    ...
    for epoch in range(epochs):
        ...
        if train_loss + valid_loss <= stop_loss:
            ...
            break
     # 训练结束后保存最佳模型
    torch.save(best_model_state, 'best_TKAN132jc2.pth')
    print("Best model saved as best_TKAN132jc2.pth")
    return fit_loss_seq, valid_loss_seq

# 训练模型
fit_loss_seq, valid_loss_seq = train_tkan()
...
best_model.load_state_dict(torch.load('best_TKAN132jc2.pth'))
best_model.eval()
