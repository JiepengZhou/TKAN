...
# 初始化模型、损失函数和优化器
model = LSTMModel(input_size, rnn_unit, output_size, dropout_prob=0.5)
...
def train_lstm():
    ...
    # 训练结束后保存最佳模型
    torch.save(best_model_state, 'best_LSTM32.pth')
    print("Best model saved as best_LSTM32.pth")
    return fit_loss_seq, valid_loss_seq

# 训练模型
fit_loss_seq, valid_loss_seq = train_lstm()
...
best_model.load_state_dict(torch.load('best_LSTM32.pth'))
best_model.eval()

with torch.no_grad():
    test_output = best_model(test_x)
    test_output = test_output.cpu().numpy()
    test_output = inverse_transform(test_output.reshape(-1, output_size), scaler_y)  # 反归一化
...
test_y = inverse_transform(test_y.reshape(-1, output_size), scaler_y)  # 反归一化
