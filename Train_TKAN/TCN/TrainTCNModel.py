...
num_channels = [32, 64, 32] # TCN每一层隐藏层的维度
kernel_size = 3
...
# 初始化模型、损失函数和优化器
model = TCN(feature_num, num_channels, kernel_size)
...
def train_tcn():
    ...
    # 训练结束后保存最佳模型
    torch.save(best_model_state, 'best_TCN32.pth')
    print("Best model saved as best_TCN32.pth")
    return fit_loss_seq, valid_loss_seq


# 训练模型
fit_loss_seq, valid_loss_seq = train_tcn()
...
best_model.load_state_dict(torch.load('best_TCN32.pth'))
best_model.eval()
